...
int main() {
  int32_t adjacency_matrix[4][4] = {0}, incident_matrix[4][4] = {0}, *powers,
          matrix_size = 4, dom, graph_size = 0;
  srand(time(NULL));
  for (int32_t i = 0; i < (matrix_size / 2); i++) {
    for (int32_t j = 0; j < matrix_size; j++) {
      adjacency_matrix[i][j] =
          rand() % 2; // Создаем связь между двумя вершинами
  ...
  //     for (int32_t j = 0; j < matrix_size; j++)
  //        incident_matrix[j] = (int32_t *)calloc(matrix_size,
  //        sizeof(int32_t));
  //  }
